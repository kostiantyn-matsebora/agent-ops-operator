...
`k8s-admin` only renders when a server that actually registers those tools
exists — which, by default, means `global.agentops.runtime.rbacMode: full` (see
below).
...
independently reviewable *by default*.

That is accepted for two reasons:

- **The safe direction — both read-only — is what every default path produces.**
- **The separation stays reachable.** `mcpServers.readOnly: true` under
  `rbacMode: full` makes this a strictly observing agent: broad grants on the
  runtime SA that nothing can exercise, and no mutating toolset renders.
...
any pod in the cluster with no credential.

`none` maps to a **readonly** server rather than to nothing, on purpose: an
agent that can read the cluster through MCP and do nothing at all through its
own identity is a useful shape, not an accident.
