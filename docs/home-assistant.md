...
`chart/charts/home-assistant/` packages the whole "the house has a problem, and
...
**The bundle ships no substrate.** No `AgentRuntime`, no floor ServiceAccount,
no LLM credential and no context volume: those are release-wide facts in the
parent chart's `global.agentops.runtimeDefaults` and `runtimes:`
([concepts](concepts.md#the-substrate-the-runtime-defaults-and-the-floor)).

It DOES render one identity per route it ships, each with no Kubernetes RBAC —
neither route touches that API.

**There is no MCP server workload either**, and that is the difference from
`kubernetes` and `prometheus`. Home Assistant serves its own MCP endpoint.
...
[Event suppression](kubernetes.md#event-suppression-eventsadaptersourcerules)
for what `rules`, `for`, `action: drop`, `escalateAfterObjects` and
`route.inhibitRules` mean, and
[What `for` actually does](kubernetes.md#what-for-actually-does) for the
verification ladder.
...
- `global.agentops.networkPolicy` restricts the admin server to runtime pods.
  Off by default.
- `global.agentops.runtimeDefaults.egressMediation` enforces each conversation's
  bound toolsets, so a control conversation is refused the admin server's tools.
  **ON by default.**
...
helm upgrade --install agentops \
  ...
  --set home-assistant.homeAssistant.credentials.controlToken="$CONTROL_TOKEN" \
  --set home-assistant.homeAssistant.credentials.operatorToken="$OPERATOR_TOKEN" \
  --set home-assistant.pipelines.enabled=true \
  --set home-assistant.pipelines.chatSources={console-ha} \
...
This is the same asymmetry `kubernetes` documents for a CLI versus the MCP
