...
|---|---|---|
| Ingest lane | `alertmanager.enabled` (**on**) | The `SignalAdapter` (`alertmanager`, reference adapter `signal-vmalertmanager/`, `port: 8080`) and — under `defaultSource.enabled` — a `SignalSource`. **Not the claim on it**: that is the wiring component, or your own `pipelines:` |
