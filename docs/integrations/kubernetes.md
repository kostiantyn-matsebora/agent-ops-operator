...
**`k8s-operate` can also search the web** (`agentops-websearch`, the built-in
`WebSearch` tool) — an unfamiliar error, a changelog, a vendor's known-issues
page. `k8s-observe` stays scoped to the cluster and does not gain it.
