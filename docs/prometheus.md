...
`chart/charts/prometheus/` packages the whole "an alert fires and an
...
floor ServiceAccount, no LLM credential Secret and no context volume here.
...
> must be restated under `prometheus:` — the render FAILS while the old
...
The toolset is **wildcarded**, unlike `kubernetes`'s enumerated lists. All six
...
an endpoint and the server component supplies one. Unlike `kubernetes`'s, there
...
--set prometheus.mcp.enabled=true \
--set prometheus.mcp.url=http://mcp-prometheus.<ns>.svc:8080/mcp \
...
`pipelines.enabled` defaults **false**, and unlike `kubernetes`'s nothing forces
...
server is read-only, so there is no second posture to express.
