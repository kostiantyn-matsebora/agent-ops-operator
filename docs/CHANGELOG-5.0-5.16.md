...
Migration guides for chart versions **5.0.0 through 5.16.0**, newest first.
Moved here from [CHANGELOG.md](CHANGELOG.md), which holds the ten most recent
versions.

Entries predate the [Keep a Changelog](https://keepachangelog.com/en/1.1.0/)
format this project now follows. They are kept **as written** — a migration
guide somebody is following must not change under them.
...
## BREAKING — adapters render, `POST /task` is removed, and the console is on by default — chart 5.0.0
