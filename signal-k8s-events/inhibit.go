...
//
// A node going NotReady makes every pod on it look broken. Without inhibition
// the node signal and a signal per pod all fire, and the pod signals are pure
// noise — they describe the same incident from the wrong end.
//
...
// Without it, one node event would silence that node's pods forever: causes
// are observed as EVENTS, and an event never announces that it stopped.
