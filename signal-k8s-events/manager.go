...
// itself.
//
// This adapter's Kubernetes access (kube.go) is for its DATA SOURCE only — the
// cluster's Events. Nothing about the contract needs it: the operator grants
// this adapter no permissions, and the cursor lives in the manager rather than
// in any adapter-side storage.
type Manager struct {
	BaseURL string
	Token   string
	HTTP    *http.Client
