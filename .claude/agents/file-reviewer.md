...
- `declares` and `references` are the whole of the cross-file review. The consolidator never opens your file: a name removed here and still in a sibling's `references` is found FROM THESE LISTS, and a name you omit is a consumer nobody checks. A prose word you include costs a read of every file that uses the word. Literal, code-shaped, exact.
