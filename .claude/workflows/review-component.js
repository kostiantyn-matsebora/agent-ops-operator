// ONE COMPONENT, READ BY A FIXED SET OF QUEUE READERS OVER A QUEUE OF FILES. Run
// inside a component's `read` job of `.github/workflows/claude-review.yml`:
// `QUEUE_READERS` `file-reviewer` subagents start at once, each with its own queue of
...
//
// WHY QUEUE READERS AND NOT A SUBAGENT PER FILE. A subagent per file paid its
// orientation and its rule reading again for every file; a queue reader pays them
...
// model turn is dropped with the turn. Every queue reader's return is validated
// against the schema at the tool layer; a file a queue reader did not return is
...
export const meta = {
  ...
  description: 'Read one component of a pull request: queue readers over a queue of files, merged into one reading',
  whenToUse: 'Only from the review: the read job runs it with the component message as args',
  phases: [
    { title: 'Read', detail: 'queue readers, each over its queue of files, one file at a time' },
...
// The runtime's concurrent-agent pool on a four-core runner. More queue readers than
// that would queue behind each other; fewer would leave a slot idle.
const QUEUE_READERS = 2
...
const READER_RETURN = { type: 'object', required: ['readings'], properties: { readings: { type: 'array', items: FILE_READING } } }

// Round-robin queues: every queue reader gets files from across the component, so
// one queue reader does not hold every large file.
const queues = Array.from({ length: Math.min(QUEUE_READERS, files.length) }, () => [])
...
${rules.map(r => '  ' + r).join('\n') || '  none'}`,
    { agentType: 'file-reviewer', label: `queue reader ${w + 1}: ${queue.length} file(s)`, phase: 'Read', schema: READER_RETURN })
})
...
log(`${ok.length} of ${files.length} file(s) read by ${queues.length} queue reader(s)` + (unread.length ? `; unread: ${unread.join(', ')}` : ''))
return {
  component,
