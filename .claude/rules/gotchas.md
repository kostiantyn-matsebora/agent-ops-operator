...
**A QUEUED `review-dispatch` RUN CAN BE DISCARDED WITH ZERO JOBS, AND
NOTHING WAS WATCHING FOR THAT SHAPE — MEASURED LIVE ON #243, 2026-09-24.**
`gate` stamps `loop:running` the instant a round starts, and `land` is the
only job that ever moves the label to a terminal value.

Two runs completed `conclusion: cancelled` with an empty `jobs` array,
superseded by a newer trigger in the same `concurrency` group before `gate`
ever started.

The label held `running` from an EARLIER round that had already posted its
own correct `capped` then `stalled` endings. Nothing downstream of either
ending ever ran again to say so.

- **This is a THIRD failure mode, distinct from the two the `timeout-minutes`
  fix above covers.** `land` runs on `fix.result` of `success`, `skipped`,
  `failure` and `cancelled` — all of which mean `gate` ran and the workflow
  reached `fix`. A run cancelled before `gate` starts never reaches ANY of
  `land`'s matched values, because the JOBS `land` depends on
  (`needs: [gate, collect, fix]`) never ran at all.
- **`refresh-loop-state.py` is the wrong tool for this, by its own design.**
  It deliberately does nothing while it sees `loop:running`, because a round
  from an earlier trigger may genuinely still be in flight — downgrading
  that label on the strength of "no thread is open right now" would misreport
  an active round as one that stopped for a person. That guard is correct
  for what it protects against, and it is exactly why it cannot also catch a
  `running` stamp whose owning run was discarded before it ever got that far.
  A stamp with no run behind it at all needs its OWN check, keyed on the
  discarded run's own shape (`cancelled`, zero jobs) rather than on live
  thread state alone, which cannot tell "genuinely running" from "orphaned"
  apart.
- **`recover-loop-state.py` + `recover-loop-state.yml` re-derive the round
  count against the cap (the same arithmetic `land-dispatch.py`'s `Round`
  class uses: `max_rounds * (1 + grants)`, from round and grant marker
  comments since the label was placed) and, only if still under the cap,
  the same live thread check `refresh-loop-state.py` uses — correcting to
  `capped` or `stalled` as appropriate, and leaving `running` alone when
  neither applies, because that shape is genuinely indistinguishable from an
  active round.
- **A SEPARATE WORKFLOW FILE, DELIBERATELY, NOT A JOB INSIDE
  `review-dispatch.yml`.** That file reacting to its own completion, inside
  its own `concurrency` group, is exactly the self-triggering surface a
  subtle bug hides in. The recovery listener shares no group with the
  workflow it audits, so it can never be cancelled by, or race, the run it
  is checking.
