...
it, where its pull request is, and what phase it is in. It SHALL NOT restate
the change's rationale, design, specifications or task list.

Its body SHALL be generated from the change rather than written by hand, so
that it cannot drift from what it points at.
...
- **THEN** its issue's phase label advances, and the transition is recorded
  once
