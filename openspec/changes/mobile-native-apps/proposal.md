...
- A new `platform/console-mobile/` component wraps that same frontend bundle
  in a native shell (Capacitor), producing three installable artifacts from
  one UI: an Android app (phone and tablet, one APK/AAB — the layout is what
...
- `platform/console-mobile/` — new component: the Capacitor project, its
  Android and iOS native projects, the connection-settings screen, and its own
  build/test tooling. Not a container image — `.github/components.sh`
  discovers components by `go.mod`/`Dockerfile`, and this component has
  neither, so it is naturally invisible to that discovery (no exclusion rule
  needed, unlike `test/`).
