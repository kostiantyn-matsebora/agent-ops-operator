...
- [ ] 1.4 Add the retired-key guard: a `vm-bundle:` key still present FAILS the
...
- [ ] 1.5 `chart/templates/NOTES.txt`: rename `index .Values "vm-bundle"` and
  every reference in that block
- [ ] 1.6 Default the adapter CR name to `alertmanager` (it is the routing key),
...
- [ ] 1.7 Verify the rename alone renders byte-identical output apart from names:
  `helm template` before and after with the bundle enabled, diffed

## 2. Metrics MCP component

- [ ] 2.1 Replace `mcp.vmlogs`/`mcp.vmmetrics` with a single `mcp` component
...
- [ ] 2.2 Keep `headers` passthrough with `valueFrom` secret refs, and the loud
  failure for an enabled component with no deployed server and no `url`
- [ ] 2.3 Document at the values why the URL is never derived: single-node VM
...
- [ ] 2.4 Settle the design's first open question: pick the Prometheus MCP server
  image, pin it, and record what it registers
- [ ] 2.5 Settle the second: enumerate the toolset's tools or wildcard
...
- [ ] 3.1 Replace `mcpServers.vmlogs`/`.vmmetrics` with one `mcpServers`
  component deploying the chosen image against a REQUIRED `backend` URL, failing
  the render when empty
- [ ] 3.2 Default the `MCPConfig` URL onto the deployed Service when the config's
  own `url` is empty; an explicit `url` still wins
- [ ] 3.3 Both components default OFF and flip together — unlike `k8s-bundle`'s,
  because there is no in-cluster endpoint to default onto. Say why in the comment
- [ ] 3.4 Render the server's own ServiceAccount and FAIL the render when it
  equals the runtime SA. Render NO RBAC for it: it reads an HTTP endpoint, not
  the Kubernetes API

## 4. Remove the logs component

- [ ] 4.1 Delete `mcp.vmlogs`, the `mcp-victorialogs` workload, and every values
  key, template branch and toolset entry that served them
- [ ] 4.2 Write the replacement into the CHANGELOG as copy-pasteable objects: the
...
- [ ] 5.1 `templates/profile.yaml`: one `AgentProfile`, identity ONLY — no
  repository, no `allowedTools`, no `mcp`, no substrate — mirroring
  `k8s-bundle/templates/profile.yaml`
- [ ] 5.2 Ship the inline `systemPrompt` role: read the alert, query the metric
...
- [ ] 5.3 `maxTurns` and an optional `runtimeRef` that emits nothing when empty,
  falling back to the runtime the parent guarantees

## 6. Wiring component

- [ ] 6.1 Create the subchart's `_helpers.tpl` — it has none today — with the
...
- [ ] 6.2 `templates/pipelines.yaml` gated on `wiringActive` AND
...
- [ ] 6.3 Refs emitted only when the object exists: the source (only under
  `alertmanager.enabled` AND `defaultSource.enabled`), the toolset and MCPConfig
  (only under `mcp.enabled`)
- [ ] 6.4 `channelRefs` only when `pipelines.channels` is non-empty — key absent,
  never null
- [ ] 6.5 Bundle label on the object, as every other bundle object carries

## 7. Post-install notes

- [ ] 7.1 Print the Alertmanager `receivers:` stanza per rendered source: the
...
- [ ] 7.2 Print what the wiring rendered when it is active, and the read-only
  nature of what it binds
- [ ] 7.3 Print the double-claim note when an install-declared pipeline also
  lists the bundle's alert source — a NOTE, never a render failure
- [ ] 7.4 Only print "nothing answers yet" when NOBODY claims the source. The
  same defect was found and fixed twice in `k8s-bundle-wiring`; do not
  reintroduce it here
- [ ] 7.5 State in the registration branch that self-registration is
  VictoriaMetrics-only and why, pointing at the printed receiver otherwise

## 8. Tests

- [ ] 8.1 `internal/integration/charttemplate_test.go`: a default install renders
  nothing from the bundle, and demo mode renders nothing either
- [ ] 8.2 A `vm-bundle:` key still present FAILS the render naming the new key
- [ ] 8.3 Bundle enabled with defaults renders no Pipeline; the source reports
  its unclaimed state
- [ ] 8.4 Wiring enabled renders exactly one Pipeline claiming the bundle's
  source with its profile, toolset and MCPConfig
- [ ] 8.5 Wiring with `profile.enabled=false` renders no Pipeline
- [ ] 8.6 `channelRefs` appears only when named; with the metrics component off,
  no ref names an object that was not rendered
- [ ] 8.7 The MCP endpoint guard and the backend guard both still fire
- [ ] 8.8 The server SA equal to the runtime SA fails the render
- [ ] 8.9 The rendered source's `spec.adapter` follows the adapter name value
- [ ] 8.10 NOTES: the receiver stanza appears with `send_resolved: false`; the
...
- [ ] 9.1 Bump `chart/Chart.yaml` (minor) and the subchart's version
- [ ] 9.2 `CHANGELOG.md`, newest first, covering every breaking edge in one
...
- [ ] 10.1 `git mv docs/vm-bundle.md docs/prometheus-bundle.md` and rewrite it:
...
- [ ] 10.2 Correct the stale claim in that page that `defaultSource.enabled` plus
...
- [ ] 10.3 `README.md` documentation index: rename the entry
- [ ] 10.4 `CLAUDE.md` map: rewrite the bundle's entry for the new component set
- [ ] 10.5 `CLAUDE.md` wiring gotcha: it names `vm-bundle` as a counter-example
...
- [ ] 10.6 `git mv openspec/specs/vm-bundle openspec/specs/prometheus-bundle` at
  archive time, so the delta applies to the renamed capability

## 11. Verification

- [ ] 11.1 `helm template` the shapes the tests assert and diff the rendered
  Pipeline, MCPConfig and profile against them
- [ ] 11.2 `helm lint`, `go vet`, `gofmt`, and the full `go test ./...` with
  envtest assets
- [ ] 11.3 Server-side dry-run against the live cluster before any apply —
  repo-side CRD validation is not proof
- [ ] 11.4 Smoke in a THROWAWAY namespace, then delete: check cluster-scoped name
