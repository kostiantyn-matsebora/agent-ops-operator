...
- **`ChannelAdapter` grows `spec.kubernetesAccess`** (mirror of the existing `SignalAdapter` field): mounts the SA token + injects `POD_NAMESPACE` so the console pod can reach the API server. Same invariant as today: the field mounts identity only; what the SA may do is granted externally.
...
- Helm chart ships the console as an opt-in bundle: `ChannelAdapter` + `Channel` CRs, read-only RBAC (Role/RoleBinding for agentops.dev CRs) against SA `agentops-channel-console`, Service, optional Ingress, and browser auth token via env (chart-provisioned Secret, `envFrom` — the manager still reads no Secrets).
...
- `channel-adapter-lifecycle`: `ChannelAdapter.spec` gains `kubernetesAccess` (parity with `SignalAdapter`) — when set, the reconciler mounts the SA token and injects `POD_NAMESPACE` into the adapter pod; default posture remains `automountServiceAccountToken: false` and the operator still creates no RBAC.

## Impact

- `api/v1alpha1/channeladapter_types.go`: add `kubernetesAccess *bool` (+ deepcopy/CRD regen).
- `internal/controller/adapterworkload.go` (shared adapter workload machinery): honor `kubernetesAccess` for ChannelAdapter as it already does for SignalAdapter.
