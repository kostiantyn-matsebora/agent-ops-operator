...
- [x] 1.1 In `signals/ha/config.go`, add domain extraction for
...
- [x] 1.2 Wire the extractor into `normalize()`: when `rec.Name ==
...
- [x] 2.1 Extend `TestIntegrationOf` (or add a sibling table-driven test) in
      ...
      shape `"Error setting up entry <title> for tuya"` with the title
      reduced to a placeholder, e.g. `someone@example.com`), a message with
      no recognizable domain falling back
...
- [x] 2.2 Add or extend a `pending_test.go` / `adapter_test.go` case
      ...
      ./...` in `signals/ha/`.
- [x] 2.3 Run the full module suite (`go build ./... && go vet ./... && go
...
- [x] 3.1 Not applicable — this change is pure log-message normalization
...
### Reference docs
- [x] 4.1 Check `docs/concepts.md` and `docs/contracts.md` for any mention of
...
- [x] 4.2 Add a `.claude/rules/gotchas.md` entry recording the pitfall: Home
...
### Adopter site
- [x] 4.3 Check `docs/integrations/home-assistant.md` for any adopter-facing
