...
- [ ] 6.5 Run the page top to bottom on a clean namespace, following only what is
      ...
      page sets is met.
      **NOT DONE — the one claim on this page that is still unverified end to
      end.** What WAS verified: every object name and flag by `helm template`
      (1.2–1.4), and a real ask against a live install, recorded in 1.5. What was
      not: a fresh `helm install` of the demo on a clean namespace, so the
      console-first flow — port-forward, sign in, New conversation — has never
      been executed as written. That needs a throwaway cluster and spends model
      credits, so it is left for the owner. The `kubectl patch` step this task
      would have exercised is gone anyway, folded into the chart by
      `console-wired-by-default`.
