...
#### Scenario: Web search is bound independently of observation

- **WHEN** a Pipeline binds the observation toolset without the web-search one
- **THEN** its conversations can read the workspace and cannot call `WebSearch`
- **WHEN** another Pipeline binds both
- **THEN** its conversations can do both, with no change to the first
