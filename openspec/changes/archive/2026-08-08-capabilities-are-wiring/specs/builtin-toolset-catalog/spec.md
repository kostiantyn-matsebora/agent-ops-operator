...
- **THEN** the observation, execution, and mutation toolsets exist and are referencable from any Pipeline's `toolsets` binding, including a profile's capability-only baseline Pipeline
...
Binding only the observation toolset on a Pipeline SHALL yield conversations whose allowlist contains the observation tools and NOT the execution tools. Because a Pipeline's bindings ARE its conversations' capabilities — profiles contribute nothing — this needs no mode and no profile edit: a profile serving several routes grants shell on one and withholds it on another purely by what each Pipeline binds.
...
#### Scenario: The baseline governs the pipeline-less paths
- **WHEN** a profile's capability-only Pipeline binds observation only
- **THEN** conversations reaching it through `POST /task` without a pipeline, or through `/<profile>` commands, can observe but not execute
