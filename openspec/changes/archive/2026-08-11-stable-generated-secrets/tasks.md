...
- [x] 6.3 Live: annotated both Secrets, then upgraded the reference install (revision 54, chart 5.8.0, via `_gitops` helmfile) — both keep their exact values AND their resourceVersion, so helm did not write them at all. Both left the release manifest; only the explicit-value Secrets (`home-ops-telegram`, `agentops-claude`) remain in it
