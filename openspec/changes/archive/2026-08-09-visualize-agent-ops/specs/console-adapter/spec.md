...
The console SHALL display all Conversations visible to its watch, distinguishing **joined** conversations (a console thread binding exists in `status.threads[]`) from **observed** ones (no console binding). Only joined conversations SHALL offer the send box; observed conversations SHALL be read-only views over CR status.
