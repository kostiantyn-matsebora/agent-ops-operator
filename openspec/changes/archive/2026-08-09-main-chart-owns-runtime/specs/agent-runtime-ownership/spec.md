...
The rendered `AgentRuntime` SHALL take `home.pvcRef` from the parent's own `persistence` block (its claim name, or `existingClaim`) with an explicit `runtime.homePvcRef` override for a claim the chart did not create. No operator SHALL have to copy a claim name between values blocks.

`runtime.idleTtlMinutes` SHALL default to empty and be omitted from the rendered CR, so the manager's `runtimeIdleTtlMinutes` is the single default and per-runtime override is deliberate.
...
- **THEN** the rendered `AgentRuntime` omits the field and runtime pods use the manager's configured idle TTL
