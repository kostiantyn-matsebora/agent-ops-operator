...
- [x] 3.4 The analysis step provisions a missing project itself, through `sonar-provision.sh <name>` with the job's token, inside the monorepo binding — a run by hand is no longer owed. The script takes names as a filter over its own list and refuses a name that is neither a component nor `scripts`. Verify: the branch's next CI run creates `agentops-scripts` and analyses it; the step's notice says so.
...
- [x] 6.1.2 The delta spec validates against the published spec (`openspec validate scripts-analysis` exits 0) so `/opsx:archive` on the branch can fold it into `openspec/specs/code-quality-analysis/spec.md`. Ticked by the archive step itself, as the last thing before the fold — the task is the delta being ready, not the archive having happened.
