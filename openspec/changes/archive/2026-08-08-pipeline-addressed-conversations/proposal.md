...
- **Chat commands address a Pipeline too.** `/<name> <task>` resolves a Pipeline, taking its profile and capabilities; the agent listing enumerates Ready Pipelines rather than profiles, since a profile name cannot be addressed. This was initially deferred to `chat-signal-origination`, but deferring it left every chat-addressed agent with no capabilities — found by auditing a live cluster, not by a test — so it lands here. `chat-signal-origination` still owns restructuring chat ORIGINATION (bare messages becoming signals); this only changes what a command addresses.
...
- `pipeline-model`: a sourceless, channelless Pipeline is no longer a declared concept; Pipelines are addressable by name — by the task API and by chat commands — and the agent listing enumerates them.
