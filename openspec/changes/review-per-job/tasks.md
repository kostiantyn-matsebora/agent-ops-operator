...
- [x] 3.1 Rewrite `.github/workflows/claude-review.yml` `queue` job (design D2): hygiene guards, then `review-input.py` (new, tested in `review-prompt.test.sh` beside `review-prompt.py`, which assembles each role's message) producing `queue` and `count` outputs and uploading `review-input.json`; `workflow_dispatch` with `number` and `dry_run` inputs, the PR resolved from the input on dispatch (design D6). Verify: `actionlint` or `python3 -c 'import yaml'` passes, and no step in `queue` runs `claude`.
