...
**Reason**: Replaced by "Reconciler owns the workload and names its identity"
below. Two of its scenarios were written around `kubernetesAccess`, which this
change deletes, and its text made the dedicated ServiceAccount part of the
posture the reconciler renders — which is the thing being moved to the chart.
Modifying it in place would leave the retired field named in scenario titles.
