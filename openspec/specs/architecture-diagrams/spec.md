...
It is one self-contained `.drawio` file. Renders are exported by hand when a
rendered image is wanted and are not committed, so no second copy can drift from
it. Its governing rule: every claim on it is checkable against this repository,
