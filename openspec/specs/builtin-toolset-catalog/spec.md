...
The chart-shipped `MCPToolset` CRs covering the runtime's built-in tools, split by risk — observation, execution, workspace mutation — so a Pipeline can bind them to widen or narrow a route's tool access without editing the AgentProfile shared by other routes.
