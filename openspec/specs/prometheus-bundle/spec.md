...
The Prometheus/Alertmanager Helm subchart composition at `chart/charts/prometheus-bundle/`: packages the Alertmanager webhook signal adapter (with VictoriaMetrics-only sender self-registration), one Prometheus query MCP configuration with its deployable server, the `alert-investigator` profile, and the bundle's own default-off wiring, as five individually toggleable components. Named for the payload format and query API it speaks rather than one backend — VictoriaMetrics answers the Prometheus query API and MetricsQL is a PromQL superset, so one server key serves both. It ships no execution substrate: the runtime, its ServiceAccount, its credential and that SA's RBAC are the parent chart's (`agent-runtime-ownership`). Self-gated, off by default, and never enabled by demo mode.
