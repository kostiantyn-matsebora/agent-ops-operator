...
(`register: {name, namespace}` required; `matchers`,
...
- **WHEN** a served source's config carries `register: {name: vmks, namespace: monitoring}` and the adapter pod has a token with write access to vmalertmanagerconfigs in `monitoring`
