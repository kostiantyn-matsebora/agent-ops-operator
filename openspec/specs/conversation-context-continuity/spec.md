...
- **Promised only where possible** — `AgentRuntime.spec.contextStorage` against
  the configured context volume. Never-promised answers fresh and says so;
...
- **WHEN** the reference runtime cannot find its session files and the install has no context volume
