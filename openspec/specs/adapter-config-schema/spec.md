...
## Purpose
Make a channel/signal type's `config` contract discoverable and checkable
without the manager learning anything about that type.

An adapter CR may declare a JSON Schema for the `config` of the
Channels/SignalSources it serves, plus the credential Secret keys it expects.
Because the declaration lives on the CR spec, it is readable the moment the CR
is applied — no registration step, and adapter binaries play no part. The
manager applies the declared document mechanically and reports the result as an
advisory condition; it never interprets config fields, and the serving adapter
remains the authority on whether a config actually works.
