# conversation-housekeeping Specification

## Purpose

The two-stage conversation lifecycle and what reclaims each stage: autoclose of finished, idle conversations into an inert `Closed` state; reopening back to `Idle` with wiring and history intact; autodelete of long-closed conversations; and the opt-in workload that reclaims orphan workspace directories and session transcripts from disk — with the ordering rules, grace period, per-run bound and dry run that make deletion sound, and the identity separation that keeps the claim root away from anything executing agent code.
