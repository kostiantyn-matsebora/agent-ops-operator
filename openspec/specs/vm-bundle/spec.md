...
The VictoriaMetrics Helm subchart composition at `chart/charts/vm-bundle/`: packages the VM Alertmanager signal adapter with its webhook Service, ships `vmlogs`/`vmmetrics` MCPConfig CRs (with optionally deployed MCP server workloads) plus a `vm-observability` MCPToolset, and bundles no profile — tool wiring targets an operator-owned profile via the Pipeline tooling stanza. Off by default and independent of demo mode.
