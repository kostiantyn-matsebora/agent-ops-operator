...
// handed to an arbitrary one, and the way to reach a specific pipeline is to
...
// the listing command is — offering an unready pipeline invites a request nothing can
...
/**
 * matchEntries answers what the typeahead should show for the current text, and
...
export function NewConversation({ onStarted }: { onStarted?: () => void }) {
  ...
  function choose(entry: VocabularyEntry) {
    const next = ADDRESS_PREFIX + entry.name + ' '
    setTask(next)
    setDismissed(true)
    setCursor(0)
  ...
  function onTaskKeyDown(e: React.KeyboardEvent<HTMLTextAreaElement>) {
    ...
    if (e.key === 'ArrowDown') {
      ...
      // Enter picks the highlighted pipeline rather than submitting: the menu is
