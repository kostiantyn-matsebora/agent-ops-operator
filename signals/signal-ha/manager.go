...
// on the manager's API port, bearer-token auth): source config, cursor state,
// and status conditions all go through this API; normalized signals go in
// through /signal/inbound and the manager applies grouping/cooldown/recurrence
// itself.
//
// This adapter holds NO Kubernetes client at all: its data source is Home
// Assistant (ha.go), reached over that instance's own API with a credential the
// reconciler projects into this pod. The cursor lives in the manager rather
// than in any adapter-side storage, which is what makes the pod disposable.
type Manager struct {
	BaseURL string
	Token   string
	HTTP    *http.Client
...
type Signal struct {
	...
	// MatchText is the log record's text, carried for RULE MATCHING and never
	// serialized (`json:"-"`, so it cannot reach the manager by accident).
	//
	// It is not a label on purpose: a message is free text with unbounded
	// cardinality, and a label carrying it would key signature grouping on the
	// exact wording — a fresh conversation per variation of one recurring
	// error. Rules still need it, because Home Assistant records carry no
	// `reason` field and level plus logger cannot separate "will retry" from
