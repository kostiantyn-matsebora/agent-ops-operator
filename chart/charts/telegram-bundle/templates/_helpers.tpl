{{- /*
The bundle renders when enabled. Unlike k8s-bundle there is no demo-mode OR, so
...
*/ -}}
{{- define "telegram-bundle.surfaceEnabled" -}}
{{- if .Values.surface.enabled -}}
{{- include "telegram-bundle.validateSurface" . -}}
true
{{- end -}}
{{- end -}}

{{- /* Everything surface.enabled makes mandatory, checked in one place. */ -}}
{{- define "telegram-bundle.validateSurface" -}}
{{- $s := .Values.surface -}}
{{- $c := $s.credentials -}}
{{- if not $s.chatId -}}
{{- fail "telegram-bundle: surface.enabled is true but surface.chatId is not set — the Telegram chat id (a forum supergroup) cannot be guessed. Set it, or set surface.enabled=false to ship the adapters only." -}}
{{- end -}}
{{- if and $c.existingSecret $c.botToken -}}
{{- fail (printf "telegram-bundle: set EITHER surface.credentials.existingSecret (%v) OR surface.credentials.botToken, not both — two sources for one bot token is ambiguous." $c.existingSecret) -}}
{{- end -}}
{{- if not (or $c.existingSecret $c.botToken) -}}
{{- fail "telegram-bundle: surface.enabled is true but no bot credential is set — give surface.credentials.existingSecret (a Secret with key botToken) or surface.credentials.botToken (the bundle then creates the Secret). Both the Channel and the router need it." -}}
{{- end -}}
{{- end -}}
...
{{- define "telegram-bundle.secretName" -}}
...
{{- define "telegram-bundle.createSecret" -}}
...
{{- define "telegram-bundle.channelTarget" -}}
