...
assert_contains "$(cat "$GH_CALLS")" 'select(.name == "docs-task")'
...
completed; printf '555 success\n' > "$JOBS_FILE"
out=$(run); rc=$?
assert_status 0 "$rc"
assert_not_contains "$(cat "$GH_CALLS")" "/rerun"
assert_contains "$out" "concluded success"

it "the LATEST attempt of the job decides, not an earlier one"
completed; printf '554 failure\n555 success\n' > "$JOBS_FILE"
